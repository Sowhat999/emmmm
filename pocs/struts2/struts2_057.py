...
def poc(url, **kwargs):
    ...
    for urls in url_list:
        ...
        try:
            res = safe_requests.get(url, timeout=timeout, headers=headers, allow_redirects=False, verify=False, )
            if res.status_code == 302 and res.headers.get('Location') is not None and str(ran_check) in res.headers.get(
                ...
                res2 = safe_requests.get(domain + urlLoca, headers=headers, timeout=6, allow_redirects=False, verify=False)
